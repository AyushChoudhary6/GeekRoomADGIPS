...
function CodeVeda() {
  ...
  const winners = [
    ...
    },
    {
      place: 'Second Place',
      team: 'Team EcoSmart',
      project: 'Sustainable City Management Platform'
    },
    {
      place: 'Third Place',
      team: 'Team EduNext',
  ...
  return (
    <div className="codeveda-page min-h-screen bg-gradient-to-br from-gray-900 to-black text-white pt-16">
      {/* Loading Overlay */}
      {showLoader && (
        ...
        <div className="hero-content relative z-10 text-center max-w-6xl px-8">
          ...
          <div className="event-stats grid grid-cols-2 md:grid-cols-4 gap-6 my-12 max-w-4xl mx-auto">
            <div className="stat-item bg-gray-800/50 border border-green-500/30 rounded-2xl text-center p-6 transition-all duration-300 hover:-translate-y-1 hover:border-green-500/60">
              <span className="stat-number block text-3xl md:text-4xl font-black text-green-400 mb-2">21</span>
              <span className="stat-label text-white/70 text-sm uppercase font-semibold">SEPTEMBER</span>
            </div>
            <div className="stat-item bg-gray-800/50 border border-green-500/30 rounded-2xl text-center p-6 transition-all duration-300 hover:-translate-y-1 hover:border-green-500/60">
              <span className="stat-number block text-3xl md:text-4xl font-black text-green-400 mb-2">150+</span>
              <span className="stat-label text-white/70 text-sm uppercase font-semibold">PARTICIPANTS</span>
            </div>
            <div className="stat-item bg-gray-800/50 border border-green-500/30 rounded-2xl text-center p-6 transition-all duration-300 hover:-translate-y-1 hover:border-green-500/60">
              <span className="stat-number block text-3xl md:text-4xl font-black text-green-400 mb-2">25</span>
      ...
      {/* Winners Section */}
      <section className="winners-section py-16 bg-gray-900/50">
        <div className="container max-w-4xl mx-auto px-4">
          <h2 className="text-4xl font-bold text-center mb-4 text-green-400">Winners</h2>
          <p className="text-white/70 text-center mb-12 max-w-2xl mx-auto">
            Celebrating the exceptional teams that pushed the boundaries of innovation and delivered outstanding solutions
          </p>

          <div className="winners-grid grid grid-cols-1 md:grid-cols-3 gap-8">
            {winners.map((winner, index) => (
              <motion.div
                ...
                className="winner-card bg-gradient-to-br from-gray-800 to-gray-900 border border-green-500/30 rounded-2xl p-8 text-center transition-all duration-300 hover:-translate-y-2 hover:shadow-lg hover:shadow-green-500/20 relative overflow-hidden"
              >
                ...
                <div className="relative z-10">
                  <div className="winner-icon flex justify-center mb-4">
                    {index === 0 ? (
                      <TrophyIcon className="w-12 h-12 text-green-400" />
                    ) : index === 1 ? (
                      <MedalIcon className="w-12 h-12 text-green-400" />
                    ) : (
                      <StarIcon className="w-12 h-12 text-green-400" />
                    )}
                  </div>

                  <div className="winner-badge text-xl font-black mb-4 text-green-400">
                    {winner.place}
                  </div>
                  <h3 className="winner-team text-xl font-bold text-white mb-3">{winner.team}</h3>
                  <p className="winner-project text-green-300 font-semibold">{winner.project}</p>
                </div>
              </motion.div>
            ))}
          </div>
